...
# Custom middleware to protect routes based on Clerk authentication
@app.middleware("http")
async def clerk_auth_middleware(request: Request, call_next):
    ...
    # Retrieve the Clerk secret key from environment variables.
    secret_key = CLERK_SECRET_KEY
    if not secret_key:
        raise Exception("CLERK_SECRET_KEY environment variable is not set.")

    # Instantiate the Clerk SDK.
    # (If desired, you could instantiate this once globally if it is thread‐safe.)
    clerk_sdk = Clerk(bearer_auth=secret_key)
    ...
    try:
        auth_result = clerk_sdk.authenticate_request(client_request, options)
    except Exception:
        # In case of error/exception you can log it and redirect the user to the login page.
        return RedirectResponse(url="/login.html")

    # If the token is invalid (or the user is not signed in) then redirect to login page.
    if not auth_result.is_signed_in:
        return RedirectResponse(url="/login.html")

    # Optionally, store the authentication payload (user information) in the request state.
    request.state.user = auth_result.payload

    # Continue processing the request if authentication succeeded.
